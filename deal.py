...
if __name__ == '__main__':
    source_dir = 'D:\Download\holstep'
    Parser = DataParser(source_dir)
    print(len(Parser.vocabulary) + 1)
    train = Parser.training_steps_and_conjectures_generator(encoding='integer', max_len=512, batch_size=64)
    validation = Parser.validation_steps_and_conjectures_generator(encoding='integer', max_len=512, batch_size=64)
    train_pivot = 0
    train_statement = 0
    train_label = 0
    validation_pivot = 0
    validation_statement = 0
    validation_label = 0
    i = 0
    for n in train:
        data = n[0]
        ...
        if i == 0:
            train_pivot = pivot
            train_statement = statement
            train_label = label
        else:
            train_pivot = np.concatenate((train_pivot, pivot))
            train_statement = np.concatenate((train_statement, statement))
    ...
    for n in validation:
        ...
        statement = data[1]
        if i == 0:
            validation_pivot = pivot
            validation_statement = statement
            validation_label = label
        else:
            validation_pivot = np.concatenate((validation_pivot, pivot))
            validation_statement = np.concatenate((validation_statement, statement))
            validation_label = np.concatenate((validation_label, label))
        i += 1
        if i % 200 == 0:
            print(i)
        if i > 1000:
    ...
    print(train_statement.shape)
    print(train_label.shape)
    print(train_pivot)
    print(train_statement)
    print(train_label)
    np.savez('train_data.npz', pivot=train_pivot, statement=train_statement, label=train_label)
    np.savez('validation_data.npz', pivot=validation_pivot, statement=validation_statement, label=validation_label)
